...
import { ClaudeApiRequest, OpenAIRequest, OpenAIMessage, StreamJsonData } from './types';
...
async function startServer(): Promise<void> {
  ...
  server.post<{ Body: ClaudeApiRequest }>('/api/claude', {
    schema: {
      body: {
        type: 'object',
        required: ['prompt'],
    ...
    await executeClaudeAndStream(prompt, sessionId || null, { 
      workspace, 
      systemPrompt, 
  ...
  server.post<{ Body: OpenAIRequest }>('/v1/chat/completions', {
    schema: {
      body: {
        type: 'object',
        required: ['messages'],
    ...
    reply.raw.write = function(chunk: any) {
          ...
          if (jsonData.type === 'system' && jsonData.subtype === 'init') {
            ...
            for (const item of content) {
              if (item.type === 'tool_result') {
                ...
                const isError = (item as any).is_error || false;
    ...
    await executeClaudeAndStream(prompt, session_id, { 
      workspace: workspace || undefined, 
      systemPrompt: systemPrompt || undefined, 
      dangerouslySkipPermissions, 
      allowedTools: allowedTools || undefined, 
      disallowedTools: disallowedTools || undefined, 
      mcpAllowedTools: mcpAllowedTools || undefined 
    }, reply);
  });
